...
#include <shared/mixture_distributions.h>
...
void test_distribution(TestData const& data, Args... args)
{
    ...
    TestSetupUniversal<TGenerator, TDistribution, glm::vec2, Grid2D<Constants::defaultVectorSize>> const setup(&distribution,
                                                                                      ss.str(), data.output, data.samples,
                                                                                      data.bufferType, data.error,
                                                                                      data.xNodes, data.yNodes,
    ...
    {
        glm::vec2 sample = i_distribution.sample(setup.buffer, setup.gen);
        Float const value = i_distribution.f(sample);
        avg += value;

        if (value > max)
            max = value;
        str_out << sample.x << ", " << sample.y << std::endl;
    }
...
int main(int argc, char** argv)
{
    ...
    test_distribution<FastLCG, BivariateNormalDistribution, IBivariateNormalDistribution>({
        .samples = samples,
        .output = "test-2d-bivariatenormal.data",
        .bufferType = bufferType, .error = error, .xNodes = 10, .yNodes = 10 },
        glm::vec2(0, 0), glm::vec2(1, 1), 0.5f, glm::vec2(-15, -15), glm::vec2(15, 15));

    test_distribution<FastLCG, HatDistribution, IHatDistribution>({
        .samples = samples,
        .output = "test-2d-hat.data",
        .bufferType = bufferType, .error = error, .xNodes = 10, .yNodes = 10 },
        glm::vec2(-3, -3), glm::vec2(3, 3));

    test_distribution<FastLCG, HurjunDistribution, IHurjunDistribution>({
        .samples = samples,
        .output = "test-2d-hurjun.data",
        .bufferType = bufferType, .error = error, .xNodes = 25, .yNodes = 25 },
        glm::vec2(0, 0), glm::vec2(2, 1));

    // 2D mixture distributions

    // Gaussian mixture model
    std::vector<glm::vec2> const mu = {
        glm::vec2(0, 0),
        glm::vec2(-3, -3),
        glm::vec2(0.5, 2.0),
        glm::vec2(-1, -3),
    ...
    std::vector<Float> const ro = { 0.0f, 0.1f, -0.5f, 0.3f, 0.6f };
    std::vector<Float> const mix = { 0.45f, 0.2f, 0.2f, 0.05f, 0.1f };
    test_distribution<FastLCG, GaussianMixtureModel, IGaussianMixtureModel>({
        .samples = samples,
        .output = "test-2d-gmm.data",
        .bufferType = bufferType, .error = error, .xNodes = 30, .yNodes = 30 },
        mu, si, ro, mix, glm::vec2(-5, -5), glm::vec2(5, 5));
