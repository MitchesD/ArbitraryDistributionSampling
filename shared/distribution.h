...
public:
    ...
    {
        x_range = std::abs(x_min_) + std::abs(x_max_);
    }
...
public:
    ...
    {
        Float const xRange = std::abs(min.x) + std::abs(max.x);
        Float const yRange = std::abs(min.y) + std::abs(max.y);
    ...
    {
        glm::vec2 u = gen->next2D();
        return { u.x * xyRange.x + min.x, u.y * xyRange.y + min.y };
    }
